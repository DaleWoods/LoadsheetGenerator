...
/**
 * Dialect-agnostic data access.
 *
 * PostgreSQL in production (Render), SQLite for local work and tests so the app
 * runs with no database server. All SQL here is written to be valid on both:
 * `?` placeholders (rewritten for pg), TEXT for ids and timestamps, INTEGER 0/1
 * for booleans.
 */
export interface Db {
